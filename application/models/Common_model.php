...
class Common_model extends CI_Model
{
	...
	function get_by_condition($table, $filter,$orders = "")
	{
		if($orders != "")
		{
	...
	function get_by_condition_2($table, $filter,$orders = "")
	{
		if($orders != "")
		{
			foreach($orders as $column => $order)
	...
	function send_mail2($from,$email,$subject,$content)
	{
		...
		/*	$config['protocol'] = "smtp";
			...
			$config['smtp_user'] = "webForm-requests@dinoflex.com";
			$config['smtp_pass'] = "SMTPPort:587";
			...
			*/
			...
			$email_arr=array($email,'sales@dinoflex.com');
			$this->email->initialize($config);
			$this->email->to($email_arr);
			$this->email->from($from['email'], $from['name']);
			$this->email->cc('system.dinoflex@gmail.com');
	...
	function upload($input = "", $folder="default", $allowd="", $thumb="",$thumb1="")
	{
		...
		foreach($folders as $dir)
		{
			if($dir != "")
			{
				if (!file_exists($structure.$dir)) {
					...
					exec('sudo -u root -S{$structure.$dir}');
		...
		if($thumb!=''){
			if (!file_exists($structure.'thumbs')) {
				...
				exec('sudo -u root -S{$structure'.'thumbs}');
			...
			if($thumb != "" && isset($thumb['width']) && isset($thumb['height']))
			{
				if($data['image_width']>$thumb['width'] && $data['image_height']>$thumb['height']){
					$thumbconfig = array(
						...
						'maintain_ratio'	=> (isset($thumb['maintain_ratio']))?$thumb['maintain_ratio']:TRUE,
						'create_thumb' 		=> TRUE,
						'thumb_marker' 		=> "",
						'width'				=> $thumb['width'],
						'height' 			=> $thumb['height']
					...
					//echo "xyz";
					//$if_maintain_ration = (isset($thumb['maintain_ratio']))?$thumb['maintain_ratio']:TRUE;
					$thumbconfig = array(
						'image_library' 	=> 'gd2',
						'source_image' 		=> $data ['full_path'],
						'new_image' 		=> $data ['file_path']."thumbs",
	...
	function upload_with_orgname($input = "", $folder="default", $allowd="", $thumb="",$thumb1="",$thumb2="")
	{
		$this->load->library('upload');
		$this->load->library('image_lib');
		$structure = './uploads/';
		...
		if($thumb2!=''){
			if (!file_exists($structure.'thumbs2')) {
				mkdir($structure.'thumbs2', 0777, true);
				exec('sudo -u root -S{$structure'.'thumbs}');
				//chmod($structure.'thumbs', 0777);
			}
			else{
				//exec('sudo -u root -S{$structure.'thumbs}');
				chmod($structure.'thumbs2', 0777);
			}
		}
		$config['upload_path']   	= $structure;
		$config['allowed_types'] 	= $allowd;
		...
		if($this->upload->do_upload($input))
		{
			...
			if($thumb1 != "" && isset($thumb1['width']) && isset($thumb1['height']))
			{
				if($data['image_width']>$thumb1['width'] && $data['image_height']>$thumb1['height']){
					$thumbconfig1 = array(
						...
						'source_image' 		=> $data ['full_path'],
						'new_image' 		=> $data ['file_path']."thumbs1",
						'maintain_ratio'	=> (isset($thumb1['maintain_ratio']))?$thumb1['maintain_ratio']:TRUE,
						'create_thumb' 		=> TRUE,
						'thumb_marker' 		=> "",
						'width'				=> $thumb1['width'],
						'height' 			=> $thumb1['height']
			...
			if($thumb2 != "" && isset($thumb2['width']) && isset($thumb2['height']))
			{
				if($data['image_width']>$thumb2['width'] && $data['image_height']>$thumb2['height']){
					$thumbconfig2 = array(
						'image_library' 	=> 'gd2',
						'source_image' 		=> $data ['full_path'],
						'new_image' 		=> $data ['file_path']."thumbs2",
						'maintain_ratio'	=> (isset($thumb2['maintain_ratio']))?$thumb2['maintain_ratio']:TRUE,
						'create_thumb' 		=> TRUE,
						'thumb_marker' 		=> "",
						'width'				=> $thumb2['width'],
				...
				$this->image_lib->initialize($thumbconfig2);
	...
	public function importData($data) {
        ...
        $res = $this->db->insert_batch('Details',$data);
        if($res){
            return TRUE;
        }else{
            return FALSE;
	...
	function get_by_condition_limit($table,$limit="",  $filters="")
	{
		$this->db->select("*");
		...
		$total = $this->db->get($table)->first_row("array");
		return array(
			"result"    => ($data->num_rows()>0)?$data->result_array():false,
